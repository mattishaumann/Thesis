...
Thesis note: this memo reflects the exploratory merge review that led to the
finalized merged-model workflow. The authoritative run used for thesis analysis
is the frozen snapshot `2026-04-03_merged_v1`, not necessarily the current
canonical merged output path if that path has been overwritten by later reruns.

In other words:

- this file documents the reasoning behind the merge decisions
- the frozen snapshot documents the exact empirical run used in the thesis
- if the canonical merged outputs drift after a rerun, restore the frozen run
