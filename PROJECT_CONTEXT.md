...
|---|-----------|--------|---------|--------|
| **H1** | Alt media focus on a narrower topic set and/or diverge from mainstream topic priorities (agenda distortion) | Agenda Distortion | Merged BERTopic, JSD, Spearman rho, Top-K overlap, Shannon entropy, coverage breadth | **In progress** (v1+v2 complete, robustness done, interpretation pending) |
| **H2** | Alt media portray democratic institutions as corrupt/failing (delegitimization) | Delegitimization | NER, Framing Analysis | **Framing annotation complete** (GPT-4o mini, normal API run); analysis pending |
...
│
├── 2a_NER/                                  # H2: Media framing annotation (NER + framing)
│   ├── Media_Framing_Batch_API_Preparation.ipynb  # Batch API prep (abandoned in favour of normal API)
│   ├── Media_Framing_Normal_API_Run.ipynb         # FINAL: full run via normal OpenAI API
│   ├── Media_Framing_Results_Analysis.ipynb       # Summary tables & pivot tables
│   ├── df_combined.csv                            # Main article dataset
│   ├── framing_codebook_prompt.txt                # GPT-4o mini classification prompt
│   ├── media_framing_batch_utils.py               # Shared helper functions
│   ├── outputs/batch_media_framing/
│   │   └── thesis_final/
│   │       ├── normal_api_run/                    # FINAL results (media_framing_thesis_sync_results.csv)
│   │       ├── analysis/                          # Outlet/category summary & pivot tables
│   │       └── batch_workflow/                    # Abandoned batch-path metadata (reference only)
│   └── archive_notebooks/                         # Legacy NER + framing notebooks
│
├── experiments/
...
### Media Framing Annotation (H2)

**Model**: GPT-4o mini (OpenAI)

**Approach**: Normal API run (synchronous) over the full thesis dataset (`2a_NER/df_combined.csv`). The Batch API path was prepared but abandoned in favour of the normal API run.

**Prompt**: `2a_NER/framing_codebook_prompt.txt` - codebook-based frame classification applied per article.

**Final results**:
- Raw annotations: `2a_NER/outputs/batch_media_framing/thesis_final/normal_api_run/media_framing_thesis_sync_results.csv`
- Outlet/label summary: `thesis_final/analysis/media_framing_thesis_outlet_label_summary.csv`
- Pivot tables (counts + shares): `thesis_final/analysis/media_framing_thesis_outlet_label_counts_pivot.csv`, `..._share_pivot.csv`

**Status**: Annotation complete. Statistical analysis and thesis write-up pending.
...
### Approach 4: GPT-4o Mini Framing Annotation (H2, Complete)

**What**: Codebook-based media frame classification applied to every article in the corpus via GPT-4o mini.

**Notebooks**: `2a_NER/Media_Framing_Normal_API_Run.ipynb` (final run), `2a_NER/Media_Framing_Results_Analysis.ipynb` (analysis)

**Method**: Synchronous OpenAI API calls using a structured codebook prompt (`framing_codebook_prompt.txt`). A Batch API path was prepared but abandoned due to workflow complexity.

**Status**: Annotation complete. Results in `2a_NER/outputs/batch_media_framing/thesis_final/`. Statistical analysis pending.
...
### After H1
- **H2 (Delegitimization)**: Framing annotation done (GPT-4o mini). Next steps: statistical analysis of frame distributions across outlets, NER for institution mentions, delegitimization framing operationalization, and thesis write-up. RT's similarity to Tagesschau in H1 makes it the key test case -- does RT cover the same topics but frame institutions negatively?
